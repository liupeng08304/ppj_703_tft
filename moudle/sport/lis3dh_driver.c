...
// u8_t Spi0WrByte(u8_t indata)
// {
//     u8_t out;
//     u8_t i;
//     for(i=0; i<3; i++)
//     {
//         if(true== spi_send(&indata, 1, &out, 1))
//         {
//             return out;
//         }
//     }
...
u8_t LIS3DH_ReadReg(u8_t Reg, u8_t* Data)
{
    ...
    // Spi0WrByte(Reg|0x80); // Select register to read from..
    //*Data = Spi0WrByte(0); // ..then read registervalue
    memset(buf,0,4);
    buf[0]=(Reg|0x80);

    if(true== spi_send(0,&buf[0], 2, &buf[2], 2))
    {
        Data[0]=buf[3];
    }
...
//     Spi0WrByte(WriteAddr&0x7f); // select register
//     Spi0WrByte(Data); // ..and write value to it..
//     ACC_CS_HIGH(); // CSN high again
//     return 1;
...
LIS3DH_STATUS_AUX_2OR, LIS3DH_STATUS_AUX_1OR,
LIS3DH_STATUS_AUX_321DA, LIS3DH_STATUS_AUX_3DA,
LIS3DH_STATUS_AUX_2DA, LIS3DH_STATUS_AUX_1DA
* Output : None
...
status_t LIS3DH_SetFilter(State_t state)
{
    u8_t value;
    value = state;
    //if( !LIS3DH_ReadReg(LIS3DH_CTRL_REG2, &value) )
...
LIS3DH_WTM_ON_INT1_ENABLE/DISABLE |
...
* Note : To enable Interrupt signals on INT1 Pad (You MUST use all input
variable in the argument, as example)
* Output : None
* Return : Status [MEMS_ERROR, MEMS_SUCCESS]
***************************************************************************
****/
...
LIS3DH_I2_BOOT_ON_INT2_ENABLE/DISABLE |
...
LIS3DH_I2_BOOT_ON_INT2_ENABLE/DISABLE |
LIS3DH_INT_ACTIVE_HIGH/LOW)
* Note : To enable Interrupt signals on INT2 Pad (You MUST use all input
variable in the argument, as example)
* Output : None
* Return : Status [MEMS_ERROR, MEMS_SUCCESS]
***************************************************************************
...
LIS3DH_YD_ENABLE/DISABLE |
LIS3DH_YS_ENABLE/DISABLE | LIS3DH_XD_ENABLE/DISABLE |
LIS3DH_XS_ENABLE/DISABLE
* example : LIS3DH_SetClickCFG( LIS3DH_ZD_ENABLE | LIS3DH_ZS_DISABLE |
LIS3DH_YD_ENABLE |
LIS3DH_YS_DISABLE | LIS3DH_XD_ENABLE |
LIS3DH_XS_ENABLE)
* Note : You MUST use all input variable in the argument, as example
* Output : None
* Return : Status [MEMS_ERROR, MEMS_SUCCESS]
...
LIS3DH_INT1_ZLIE_ENABLE/DISABLE...
* Output : None
* Note : You MUST use all input variable in the argument, as example
* Return : Status [MEMS_ERROR, MEMS_SUCCESS]
***************************************************************************
...
status_t LIS3DH_FIFOModeEnable(LIS3DH_FifoMode_t fm)
{
    ...
    if(fm == LIS3DH_FIFO_DISABLE)
    {
        ...
        if( !LIS3DH_WriteReg(LIS3DH_FIFO_CTRL_REG, value) ) //fifo modebypass
            return MEMS_ERROR;
        if( !LIS3DH_ReadReg(LIS3DH_CTRL_REG5, &value) )
            return MEMS_ERROR;
        value &= 0xBF;
...
LIS3DH_STATUS_REG_YOR, LIS3DH_STATUS_REG_XOR,
LIS3DH_STATUS_REG_ZYXDA, LIS3DH_STATUS_REG_ZDA,
LIS3DH_STATUS_REG_YDA, LIS3DH_STATUS_REG_XDA,
...
/*************************************************************************
******
* Function Name : LIS3DH_GETRefVal
* Description : Get the acceleration value from reference register
* Input : Reference register value
* Output : None
* Return : Status [MEMS_ERROR, MEMS_SUCCESS]
***************************************************************************
****/
status_t LIS3DH_GETRefVal(u8_t* val)
{
    u8_t value;
    if( !LIS3DH_ReadReg(LIS3DH_REFERENCE_REG, &value) )
        return MEMS_ERROR;
    *val=value;
    return MEMS_SUCCESS;
}
...
bool kx023softInit(void)
{
    uint8_t value;

    if( !LIS3DH_WriteReg(0x7f, 0) )
        return MEMS_ERROR;

    if( !LIS3DH_WriteReg(0x19, 0) )//register2
        return MEMS_ERROR;

    if( !LIS3DH_WriteReg(0x19, 0x80) )//register2
        return MEMS_ERROR;
    delay_ms(50);


    if( !LIS3DH_ReadReg(0x0f, &value) )//who i am
        return MEMS_ERROR;

    if(0x15!=value)
        return MEMS_ERROR;

    if( !LIS3DH_ReadReg(0x0c, &value) )//
        return MEMS_ERROR;
    ...
    if( !LIS3DH_WriteReg(0x1a, 0x25) )//tilt 1.563hz   tap 12.5  output 25
        return MEMS_ERROR;

    if( !LIS3DH_WriteReg(0x1b, 0x01) )//register3  25hz
        return MEMS_ERROR;

    if( !LIS3DH_WriteReg(0x1d, 0x00) )//no interrupt
        return MEMS_ERROR;

    if( !LIS3DH_WriteReg(0x1e, 0x00) )//no interrupt
        return MEMS_ERROR;

    if( !LIS3DH_WriteReg(0x3a, 41) )// 41 sample
        return MEMS_ERROR;

    if( !LIS3DH_WriteReg(0x3b, 0xc1) )//stream
        return MEMS_ERROR;
...
bool kx023Init(bool on_flg)
{
    uint8_t i,value;
    uint8_t returnvalue=1;
    EnableSpi(true,SPI_MODE_G,SPI_MISO_PIN,SPI_MOSI_PIN,SPI_SCK_PIN);


    if(true==on_flg)
    {

        returnvalue&=LIS3DH_ReadReg(0x18, &value);
        ...
        returnvalue&=LIS3DH_WriteReg(0x18, value) ;
        delay_ms(50);
        while(MEMS_SUCCESS!=kx023softInit())
        {
            delay_ms(100);
            i++;
            if(i>3)
            {
                ...
                EnableSpi(false,SPI_MODE_G,SPI_MISO_PIN,SPI_MOSI_PIN,SPI_SCK_PIN);
                return false;
            }
        }
    }
    returnvalue&=LIS3DH_ReadReg(0x18, &value);  //resigster 1
    if(true!=on_flg)
    {
        value&=0x7f;//进入休眠
    }
    ...
    returnvalue&=LIS3DH_WriteReg(0x18, value);//register2
    returnvalue&=LIS3DH_ReadReg(0x18, &value) ;  //resigster 1
    EnableSpi(false,SPI_MODE_G,SPI_MISO_PIN,SPI_MOSI_PIN,SPI_SCK_PIN);
    delay_ms(50);
    if(MEMS_ERROR==returnvalue)
        return MEMS_ERROR;
    return MEMS_SUCCESS;
...
bool AccSensorConfig(void)
{

    uint8_t value[4];
    uint8_t returnvalue,i,j;
    EnableSpi(true,SPI_MODE_G,SPI_MISO_PIN,SPI_MOSI_PIN,SPI_SCK_PIN);
    nrf_gpio_cfg_output(SPI_CS_3DH);
    ...
    } while( (value[0]!=0x33)&&((value[0]!=0x15)));
    ...
    for(i=0; i<5; i++)
    {
        returnvalue=LIS3DH_SetMode(LIS3DH_POWER_DOWN);

        for(j=0; j<3; j++)
        {
            delay_ms(200);
            returnvalue&=LIS3DH_SetMode(LIS3DH_NORMAL); //
            if(returnvalue==1)
                break;
        }

        returnvalue&=LIS3DH_SetAxis(LIS3DH_X_ENABLE|LIS3DH_Y_ENABLE|LIS3DH_Z_ENABLE);
        returnvalue&=LIS3DH_SetODR(LIS3DH_ODR_25Hz); // ? ??? LIS3DH_ODR_25Hz
        returnvalue&=LIS3DH_SetFullScale(LIS3DH_FULLSCALE_2); // ? ???+-2g
        returnvalue&=LIS3DH_SetBLE(LIS3DH_BLE_LSB); // ? ? ? ??????
...
void SetPowerMode(LIS3DH_Mode_t md)
{
...
uint8_t GetData(AxesRaw_t* axes)
{
    ...
    if(iskx023==true)
    {
        LIS3DH_ReadReg(0x3c, &times);
        pdata=(uint8_t*)&axes[0];
        for(i=0; i<times; i++)
        {
            LIS3DH_ReadReg(0x3f,pdata+i%(192));
        }
        times/=6;
    } else
    {
...
void SetAccPowerOff(bool off_flg)
{
    AxesRaw_t adx[32];
    if(true==off_flg)
    {
        ...
        if(iskx023==true)
            ...
            SetPowerMode(LIS3DH_POWER_DOWN);
    ...
    } else
    {
        if(iskx023==true)
            ...
            SetPowerMode(LIS3DH_NORMAL);
    }

}

#define FRE_3DH_MS  40

#include "app_button.h"
#include "nrf_log.h"

void acc_click_irq(nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{

//	LIS3DH_GetClickResponse(&res);
	       SendSem(PRESS_KEY);

}


void print_ket_state(void)
{
...
void Acc__EnableClickDetect(void)
{
  ...
  LIS3DH_SetClickCFG(LIS3DH_XS_ENABLE|LIS3DH_YS_ENABLE|LIS3DH_ZS_ENABLE);//|LIS3DH_YS_ENABLE|LIS3DH_XS_ENABLE
  LIS3DH_SetClickTHS(10);//CLICK-CLICK threshold
  LIS3DH_SetClickLIMIT(100/FRE_3DH_MS);//CLICK-CLICK Time Limit
	LIS3DH_SetClickLATENCY(800/FRE_3DH_MS);
 // LIS3DH_SetInt1Threshold(0x02);
  LIS3DH_SetInt1Pin(LIS3DH_CLICK_ON_PIN_INT1_ENABLE);
	
	 int_on_pin(14,acc_click_irq);
